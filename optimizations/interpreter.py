...
#random.choice(f) if depth<3 else 
    ...
    def _randexp(atom, depth=2):
        ...
        if atom in F:
            return (atom, *[_randexp(
                random.choice(f) if depth<maxdepth else random.choice(list(T)), depth+1
...
def evolve(functions, terminals, fitness_function, pop_size=50, init_max_depth=10, crossover_rate=0.5):  
    ...
    fig, ax = plt.subplots()
    ax.plot(xs, ys)
    ax.set(xlabel='individuals processed', ylabel='max fitness',
