...
def interpret(exp, definitions=None):
    ...
    try:
        return head(exp)(*map(lambda e: interpret(e, definitions), tail(exp)))
...
def _simplify(exp):
    if type(exp) in (int, str, float):
        return exp
    if all((type(i) in (str, int, float)) for i in tail(exp)) and any(
        (type(i) is str) for i in tail(exp)
    ):
        return exp
    try:
        return head(exp)(*map(simplify, tail(exp)))
    except OverflowError:
        return 0
...
def simplify(exp):
    if type(exp) in (tuple, list):
        return wrapper(head(exp))(*map(simplify, tail(exp)))
    else:
        return exp
...
def with_subindex(exp, idxs, newval):
    if len(idxs) == 0:
        ...
        return [
            *exp[: idxs[0]],
            with_subindex(exp[idxs[0]], idxs[1:], newval),
            *exp[idxs[0] + 1 :],
        ]
        ...
        try:
            ...
            while len(new_pop) < pop_size:
                ...
                if rand < (mutation_rate + crossover_rate):
                    if random.random() < (
                        mutation_rate / (mutation_rate + crossover_rate)
                    ):
                        new_pop.append(
                            mutate(
